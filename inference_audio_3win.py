...
checkpoint = "/disk1/projects/sonar_multilingual_segmentation/checkpoints/sonar/sonar_audio_window_prev_curr_next_lstm1.ckpt"  # Best checkpoint
...
def main(cfg: DictConfig) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    for file in files:
        ...
        print(f"Found {audio.shape[-1] / sr} sec of audio file.")
        # Resample to 16k
        ...
        for spl, pred in zip(audio_splits, preds):
            if pred==1:
                ...
                out.append(spl)
        print(preds)
        torchaudio.save(output_root / file.name.replace(".", "_prev_curr_next."), torch.cat(out, dim=1), sample_rate=sr, bits_per_sample=16)
        print("Saved!")
